...
export async function discoverFederatedCapabilityOffers(request, providers, options = {}) {
  exact(request, ["kind", "requestId", "operation", "observer", "evaluatedAt", "maximumCandidates", "endpoints"], "request");
  ...
  if (!Number.isSafeInteger(request.maximumCandidates) || request.maximumCandidates < 1 || request.maximumCandidates > (options.maximumCandidates ?? 128)) {
    fail("maximumCandidates is outside the admitted bound");
  }
  if (!Array.isArray(request.endpoints) || request.endpoints.length < 1 || request.endpoints.length > (options.maximumEndpoints ?? 64)) {
    fail("one bounded endpoint denominator is required");
  }
  ...
  const result = Object.freeze({
    ...
    candidates: candidates.slice(0, request.maximumCandidates),
    refusals,
  });
